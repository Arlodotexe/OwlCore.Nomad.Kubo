...
In Nomad, the state of an object is rebuilt from a seed by replaying and aggregating individual event streams published by individual devices, each containing data on interactions with an application domain on that device. Even as peers come and go, publishing only peer-specific interactions with the application domain allows the aggregated state to eventually converge on each device without CDRTs or generalizable conflict resolution logic. Conflict resolution is effectively delegated to an implementation detail. 
